...
public struct OrbContainer: View {
    ...
    /// Largest file Murmur will read off a drop.
    ///
    ...
    public var body: some View {
        OrbView(activity: controller.activity, isTargeted: isTargeted) {
    ...
    private func speakContents(of url: URL) {
        ...
        guard readableTypes.contains(url.pathExtension.lowercased()) else {
            controller.reportDropRejected("Murmur can read plain text files — try a .txt or .md.")
            return
        }
