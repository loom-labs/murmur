...
public struct OrbContainer: View {
    ...
    /// Largest file Hugo will read off a drop.
    ///
    ...
    private let controller: HugoController
    ...
    private func speakContents(of url: URL) {
        ...
        guard readableTypes.contains(url.pathExtension.lowercased()) else {
            controller.reportDropRejected("Hugo can read plain text files — try a .txt or .md.")
            return
        }
