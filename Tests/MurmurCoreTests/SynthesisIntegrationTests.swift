...
///
/// Skipped unless `MURMUR_INTEGRATION=1`. Run locally with:
///
///     MURMUR_INTEGRATION=1 swift test --filter SynthesisIntegrationTests
@Suite(
...
struct SynthesisIntegrationTests {
    ...
    func streamsChunksInOrder() async throws {
        ...
        let text = """
            Murmur runs entirely on your own machine. It uses Parakeet for \
