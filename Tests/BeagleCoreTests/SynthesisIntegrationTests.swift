...
///
/// Skipped unless `BEAGLE_INTEGRATION=1`. Run locally with:
///
///     BEAGLE_INTEGRATION=1 swift test --filter SynthesisIntegrationTests
@Suite(
...
struct SynthesisIntegrationTests {
    ...
    func streamsChunksInOrder() async throws {
        ...
        let text = """
            Beagle runs entirely on your own machine. It uses Parakeet for \
